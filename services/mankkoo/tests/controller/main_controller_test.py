...
def test_savings_distribution(test_client):
    # GIVEN
    view = [
        {"type": "checking", "total": 3467.07, "percentage": 0.3467},
        {"type": "savings", "total": 9740.01, "percentage": 0.2974},
        {"type": "cash", "total": 0, "percentage": 0},
        {"type": "stocks", "total": 2590.3, "percentage": 0.259},
        {"type": "retirement", "total": 969, "percentage": 0.0969},
    ]
...
def insert_view(name: str, content: dict | list):
    # not sure why this is needed since before each test db is cleaned up
    db.execute("TRUNCATE views;")
    db.execute(
        f"INSERT INTO views (name, content) VALUES ('{name}', '{json.dumps(content)}'::jsonb);"
    )
