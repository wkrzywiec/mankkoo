...
from mankkoo.config import DevConfig, ProdConfig
from mankkoo.controller.account_controller import account_endpoints
from mankkoo.controller.admin_controller import admin_endpoints
from mankkoo.controller.investment_controller import investment_endpoints
...
def create_app(app_config=None):
    ...
    app = APIFlask(
        __name__,
        title="Mankkoo - 'accounting' service API",
        version="1.0",
...
def handle_notification(notify):
    log.info(
        f"Received notification. Channel: '{notify.channel}'. Payload: '{notify.payload}'"
    )
