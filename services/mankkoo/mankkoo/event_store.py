...
class Event:
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.version == other.version
            and self.occured_at == other.occured_at
...
def create(streams: list[Stream]):
    log.info("Storing {len(streams)} stream(s)...")
...
def load_latest_event_id(stream_id: str) -> Event:
    ...
    with db.get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                f"SELECT e.id, e.stream_id, e.type AS event_type, e.data, e.version, e.occured_at, s.type AS stream_type FROM events e JOIN streams s ON e.stream_id=s.id WHERE e.stream_id = '{str(stream_id)}' ORDER BY version DESC LIMIT 1"
            )
            row = cur.fetchone()
...
def get_stream_by_metadata(key: str, value) -> Stream | None:
    log.info(
        f"Loading stream by its matadata property key '{key}' and value '{value}'..."
    )
    with db.get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
...
def get_stream_metadata(stream_id: UUID) -> dict:
    log.info("Loading stream's '{stream_id}' metadata...")
    with db.get_connection() as conn:
        with conn.cursor() as cur:
...
def get_all_streams() -> dict:
    ...
    with db.get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT metadata-> 'accountNumber' AS account_number, id FROM streams;"
            )
            rows = cur.fetchall()
