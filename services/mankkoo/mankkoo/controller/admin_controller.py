...
from apiflask.fields import String, Date, List, Raw
...
@admin_endpoints.doc(summary='Update Views', description='Updates all views based starting from specified date until the latest event.')
...
@admin_endpoints.doc(summary='Load All Views', description='Retrieves all available view keys')
def load_all_views():
    try:
        return {
            'views': [
        ...
        return {
            'success': 'false',
            'message': f'Error loading views: {str(e)}'
        }, 500
...
@admin_endpoints.doc(summary='Load Specific View', description='Loads a specific view by its name')
